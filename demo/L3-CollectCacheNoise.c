 ...
 * and text files are used for easier use by other programs.
 */
...
#define SAMPLES 10010

int main(int ac, char **av)
{
    FILE *fpCsv = fopen("NoiseData/L3-NoiseData.csv", "w");
    if (fpCsv == NULL)
    {
        return 0;
    }

    FILE *fpTxt = fopen("NoiseData/L3-NoiseData.txt", "w");
    if (fpTxt == NULL)
    {
        return 0;
    }
    ...
    for (int i = 0; i < Round; i++)
    {
        ...
        SetTotal = 0;
        SetTotal += res[(SAMPLES - 1) * nmonitored + nmonitored - 1];
        fprintf(fpCsv, "%d\n", SetTotal);
        putchar('\n');
        fprintf(fpCsv, "\n\n");
